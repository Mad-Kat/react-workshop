/**
 * Exercise 02: State as a Snapshot & the Key Trick — SOLUTIONS
 * =============================================================
 */
...
// ---------------------------------------------------------------------------
// Solution C: Stale Closure Demo
//
// The closure captures `count` at the time the click handler runs.
// After `setCount(count + 1)`, the `count` variable hasn't changed yet —
// it still holds the old snapshot value. The setTimeout callback reads
// this stale snapshot 2 seconds later.
//
// Fix: use a ref to always read the latest value at call time. Keep the ref
// in sync on every render by assigning `countRef.current = count`.
// ---------------------------------------------------------------------------

export const StaleClosureDemo: FunctionComponent = () => {
  const [count, setCount] = useState(0);
  const countRef = useRef(count);
  countRef.current = count; // Always keep ref in sync with latest render

  const incrementAndLog = () => {
    setCount((prev) => prev + 1);
    setTimeout(() => {
      // Read from the ref to get the latest value, not the stale closure
...
//      value changes.
//   4. Stale closures happen because functions capture values at creation time.
//      A ref always points to the latest value regardless of when it is read.
// ---------------------------------------------------------------------------
